            ...
            <div class="modal-header">
                ...
                <div class="modal-title">
                    <div class="content-title">
                        <h3 class="text-center">Sửa bài học</h3>
                    </div>
                </div>
            </div>
            <form id="form-insert-lession" class="form-insert form-horizontal" novalidate method="POST" action="<c:url value='/Admin/Course/ListCourse/Lession/Edit'/>">
                <div class="modal-body"> 
                	<input type="hidden" name="id" value="${SELECTED_LESSION.id}">
                    <div class="form-group">
                        <label for="name" class="control-label col-sm-3">Tên bài học</label>
                        <div class="col-sm-9">
                            <input type="text" id="name" required name="name" value="${SELECTED_LESSION.name}" onkeyup="$('#nameAscii[readonly]').val(removeUnicodeURL($(this).val()))" class="form-control" placeholder="">
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="name" class="control-label col-sm-3">Miêu tả</label>
                        <div class="col-sm-9">
                            <textarea  id="description" required name="description"  class="form-control" placeholder="">${SELECTED_LESSION.description}</textarea>
                        </div>
                    </div>
                     <div class="form-group">
                        ...
                        <div class="col-sm-9">
                        	<select class="form-control" required name="lessionType">
                                ...
                                <c:forEach items="${f:findAllLessionType()}" var="lessionType">
                                    <option ${SELECTED_LESSION.lessionType.id ==lessionType.id?"selected":""}   value='${lessionType.id}'>${lessionType.name}</option>
                                </c:forEach>
                            </select>
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <input type="text" id="nameAscii" required name="nameAscii" value="${SELECTED_LESSION.nameAscii}" readonly class="form-control" placeholder="">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-9">
                            <input type="number" id="cssClass" name="orderDisplay" class="form-control" value="${SELECTED_LESSION.orderDisplay}">
                        </div>
                    </div>
                     <div class="form-group">
                        <label class="control-label col-sm-3">PRO</label>
                        <div class="col-sm-9">
                            <input type="checkbox"  ${SELECTED_LESSION.isPro?"checked":""} id="isPro" name="isPro">
                        </div>
                    </div>
                     <div class="form-group">
                        <label class="control-label col-sm-3">Hiển thị</label>
                        <div class="col-sm-9">
                            <input type="checkbox" ${SELECTED_LESSION.isActive?"checked":""} id="isActive" name="isActive">
                        </div>
                    </div>
                </div>
