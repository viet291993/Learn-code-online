...
function init_PNotify() {
    "undefined" != typeof PNotify && (console.log("init_PNotify"), new PNotify({
        title: "Xin chào",
        type: "info",
        text: "Chúc bạn một ngày làm việc tốt lành",
        nonblock: {
            nonblock: 0
        },
        addclass: "dark",
...
$.fn.extend({
    treed: function(o) {
        ...
        tree.find('li').has("ul").each(function() {
            ...
            branch.children("ul:first").toggle(true);
        });
...
$('#tree').treed({
    openedClass: 'glyphicon glyphicon-expand',
    closedClass: 'glyphicon glyphicon-collapse-down'
});
...
$.fn.extend({
    treeview: function(o) {
        ...
        tree.find('.syllabus').each(function() {
            var branch = $(this); 
            var chilbranch = branch.children('li:first');
            chilbranch.prepend("<i class='indicator glyphicon " + closedClass + "'></i>");
            chilbranch.addClass('branch');
            chilbranch.on('click', function(e) {
                if (this == e.target) {
                    var icon = $(this).children('i:first');
                    icon.toggleClass(openedClass + " " + closedClass);
                    branch.children("div:first").toggle();
                }
            })
            branch.children("div:first").toggle();
        });
        tree.find('.lession').each(function() {
            var branch = $(this); 
            var chilbranch = branch.children('li:first');
            chilbranch.prepend("<i class='indicator glyphicon " + closedClass + "'></i>");
            chilbranch.addClass('branch');
...
$(document).on('click', '#addInstruction', function () {
	var orderOld = $('#instruction').find('.panel:last').find('input:last').val();
	var order = 1;
	if(orderOld!= null){
		order = Number(orderOld) + 1;
	}
	$('#instruction').append('<div class="panel"><a class="panel-heading" role="tab" id="headingOne" data-toggle="collapse" data-parent="#accordion" href="#'+order+'" aria-expanded="true" aria-controls="collapseOne"><h4 class="panel-title">Yêu cầu '+order+'<i class="fa fa-trash pull-right deleteInstruction"></i></h4></a><input type="hidden" name="insID"><div id="'+order+'" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="headingOne" aria-expanded="true" style=""><div class="panel-body"><div class="form-group"><label for="name" class="control-label col-sm-3">Nội dung</label><div class="col-sm-9"><textarea id="editor'+(dd+1)+'"  required name="insContent" class="form-control editor" placeholder=""></textarea></div></div><div class="form-group"><label for="name" class="control-label col-sm-3">Hướng dẫn</label><div class="col-sm-9"><textarea id="editor'+(dd+2)+'"  id="insHint" required name="insHint" class="form-control editor" placeholder=""></textarea></div></div><div class = "form-group"><label for="name" class="control-label col-sm-3">Code yêu cầu</label><div class="col-sm-9"><textarea id="insRequiredCode" name="insRequiredCode"  class="form-control" placeholder=""></textarea></div></div><div class="form-group"><label for="name" class="control-label col-sm-3">Kết quả</label><div class="col-sm-9"><textarea  id="insResult" name="insResult" class="form-control" placeholder=""></textarea></div></div><div class="form-group"><label class="control-label col-sm-3">Thứ tự</label><div class="col-sm-9"><input type="number" id="insOrderDisplay" data-name="insOrderDisplay" name="insOrderDisplay" class="form-control" value="'+order+'"></div></div></div></div></div>');
	CKEDITOR.replace( 'editor'+(dd+1),{
		   language: 'vi',
	       height: 200
	   }); 
	CKEDITOR.replace( 'editor'+(dd+2),{
		   language: 'vi',
	       height: 200
	   }); 
	   dd=dd+2; 
...
$(document).on('click', '#addQuiz', function () {
	$('#quiz').append('<div class="form-group"><input type="radio" data-name="isTrue" class="col-sm-2 mt-10"  name="Quiz"><input type="hidden" data-name="quizId"><div class="col-sm-7"><input type="text" data-name="answer" class="form-control"></div><a class="text-danger" href="javascript:void(0)"><i class="fa fa-trash deleteQuiz col-sm-3 pt-10"></i></a></div>');
});
